...
class FieldNDVI(object):
    def __init__(self, map_widget):
        ...
        # Datepicker control
        sd = widgets.DatePicker(description="Start date", disabled=False)
        ed = widgets.DatePicker(description="End date", disabled=False)
        date_box = widgets.VBox([sd, ed])
        self.dt_control = ipyleaflet.WidgetControl(
            widget=date_box, position="bottomleft"
        )
    ...
    def get_s2_collection(self, *args, **kwargs):
        s2_new = wf.ImageCollection.from_id(
            "sentinel-2:L1C",
        ...
        s2_new = s2_new.mask(cdl.mosaic() != 54) # 54 is the tomato CDL category identifier
    ...
    def plot_timeseries(self, *args, **kwargs):
        ...
        target_dates = np.arange(dates.min(), dates.max(), datetime.timedelta(days=1))
    ...
    def calculate(self, *args, **kwargs):
        ...
        print("Starting Calculation with ", + last_draw["geometry"])
